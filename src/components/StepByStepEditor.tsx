...
const StepByStepEditor: React.FC = () => {
  ...
  const handleSubmit = () => {
    const cardId = createCardFromTemplate({
      ...
      styles: {
        ...
        position: formData.namePosition,
        textAlignment: 'center'
      }
    });
  ...
  const renderNameAndJob = () => {
    const [draggingField, setDraggingField] = useState<'name' | 'job' | null>(null);

    const handleDragStart = (field: 'name' | 'job') => (e: React.MouseEvent) => {
      e.stopPropagation();
    ...
    const handleDrag = (e: React.MouseEvent) => {
      ...
      setFormData(prev => ({
        ...prev,
        [draggingField === 'name' ? 'namePosition' : 'jobPosition']: {
          vertical: Math.min(Math.max(y, 0), 100),
          horizontal: Math.min(Math.max(x, 0), 100)
        }
      }));
    };
        ...
        <div className="mb-6">
          ...
          >
            ...
            <div 
              ...
              style={{
                top: `${formData.namePosition.vertical}%`,
                left: `${formData.namePosition.horizontal}%`,
            ...
            <div 
              className="absolute whitespace-nowrap text-center cursor-move"
              style={{
                top: `${formData.jobPosition.vertical}%`,
                left: `${formData.jobPosition.horizontal}%`,
                transform: 'translate(-50%, -50%)',
                padding: `${formData.textPadding.vertical}px ${formData.textPadding.horizontal}px`
              }}
              onMouseDown={handleDragStart('job')}
            >
              <p 
                className="text-white"
                style={{ 
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  min="0"
                  max="100"
                  value={formData.namePosition.vertical}
                  onChange={(e) => setFormData({
                    ...formData,
                    namePosition: {
                      ...formData.namePosition,
                ...
                <span className="text-sm text-gray-600 w-12">
                  {formData.namePosition.vertical}%
                </span>
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  min="0"
                  max="100"
                  value={formData.namePosition.horizontal}
                  onChange={(e) => setFormData({
                    ...formData,
                    namePosition: {
                      ...formData.namePosition,
                ...
                />
                <span className="text-sm text-gray-600 w-12">
                  {formData.namePosition.horizontal}%
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setFormData({ ...formData, nameSize: 16 })}
                ...
                <button
                  onClick={() => setFormData({ ...formData, nameSize: 64 })}
                  className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300"
                >
                  كبير
                </button>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setFormData({ ...formData, fontSize: 16 })}
                  className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300"
                >
                  صغير
                </button>
                ...
                <button
                  onClick={() => setFormData({ ...formData, fontSize: 64 })}
                  className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300"
                >
                  كبير
                </button>
